...
describe('SessionDetailModal Component', () => {
  ...
  it('renders session details after loading', async () => {
    ...
    expect(screen.getAllByText((_, el) => el?.textContent?.includes('100kg x 10') ?? false).length).toBeGreaterThan(0);
    
    expect(screen.getByText('Squat')).toBeInTheDocument();
    expect(screen.getAllByText((_, el) => el?.textContent?.includes('120kg x 5') ?? false).length).toBeGreaterThan(0);
  });

  it('handles empty exercise name gracefully', async () => {
    const mockDetails = [
      { exercises: null, weight: 50, reps: 10, rpe: 5 },
    ];
    vi.mocked(sessionService.fetchSessionDetails).mockResolvedValue({ data: mockDetails } as any);
