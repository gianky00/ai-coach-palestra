...
describe('WorkoutSummaryModal Component', () => {
  ...
  it('renders summary values correctly', () => {
    ...
    expect(screen.getAllByText((_, el) => el?.textContent?.replace(/[,.]/g, '').includes('5000 kg') ?? false).length).toBeGreaterThan(0); // Volume
    expect(screen.getAllByText('20').length).toBeGreaterThan(0);    // Sets
    expect(screen.getAllByText((_, el) => el?.textContent?.includes('60 min') ?? false).length).toBeGreaterThan(0);    // Duration
