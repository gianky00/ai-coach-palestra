    ...
    return {
        ...
        OggiView: ({ setShowAddEx, setSelectedEx }: any) => (
          <div data-testid="oggi-view">
            <button data-testid="open-add-ex" onClick={() => setShowAddEx(true)}>Open Add Ex</button>
            <button data-testid="open-log-ex" onClick={() => setSelectedEx({ id: 'ex-1' })}>Open Log Ex</button>
          </div>
        ),
...
describe('App Root Component', () => {
    ...
    it('renders main app when authenticated (OggiView is default)', async () => {
        vi.mocked(useAuth).mockReturnValue({ session: { user: mockUseWorkoutData.user }, loading: false } as any);
        render(<App />);
    ...
    it('shows timer overlay when timer is active', async () => {
        ...
        vi.mocked(useAuth).mockReturnValue({ session: { user: mockUseWorkoutData.user }, loading: false } as any);
        vi.mocked(useTimer).mockReturnValue({ ...mockUseTimer, timerActive: true, timer: 65, setTimerActive: mockSetTimerActive } as any);
