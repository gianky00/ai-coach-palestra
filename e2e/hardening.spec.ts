...
test.describe('Extreme Coverage & Hardening', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      const projectRef = 'ekckzmihqswqfglowpwk'; // Mock fallback
      const session = {
      ...
      // Cerchiamo di capire il projectRef reale dall'URL se possibile o iniettiamo un valore generico
    });

    await page.route(supabasePattern, async (route) => {
        const url = route.request().url();
        if (url.includes('/auth/v1/user')) {
            return route.fulfill({ status: 200, body: JSON.stringify({ id: userId, email: 'test@example.com' }) });
        }
        if (url.includes('/rest/v1/exercises')) {
            return route.fulfill({ status: 200, body: JSON.stringify([{ id: '1', name: 'Panca Piana', muscle_group: 'Petto', target_sets: 3, target_reps: '10' }]) });
        }
        if (url.includes('/rest/v1/profiles')) {
            return route.fulfill({ status: 200, body: JSON.stringify([{ id: userId, full_name: 'Test User' }]) });
        }
  ...
  test('Security: XSS Protection in Input Fields', async ({ page }) => {
    ...
    const exerciseCard = page.locator('.ex-name-premium', { hasText: 'script' });
