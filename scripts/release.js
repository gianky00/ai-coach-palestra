...
if (process.env.VERCEL === '1' || !fs.existsSync(path.join(__dirname, '..', '.git'))) {
  console.log('\x1b[33mAmbiente Vercel o assenza di repository Git rilevata. Salto dello script di release.\x1b[0m');
  process.exit(0);
}
...
if (isCi) {
  ...
  try {
    ...
    if (commitMsg.includes('chore(release):')) {
      console.log(
        '\x1b[33mUltimo commit di release già elaborato. Uscita preventiva per evitare loop.\x1b[0m',
      );
      process.exit(0);
    }

    let type = 'patch';
    if (commitMsg.startsWith('feat:') || commitMsg.startsWith('feat(')) {
      type = 'minor';
    } else if (commitMsg.startsWith('fix:') || commitMsg.startsWith('fix(')) {
      type = 'patch';
    } else if (commitMsg.includes('BREAKING CHANGE') || commitMsg.includes('major:')) {
      type = 'major';
    }

    const nextVersion = bumpVersion(currentVersion, type);
    console.log(`Tipo di incremento rilevato: \x1b[32m${type}\x1b[0m`);
    console.log(`Nuova versione calcolata: \x1b[32mv${nextVersion}\x1b[0m\n`);
    ...
    console.error('\x1b[31mErrore durante il processo di release CI:\x1b[0m', error);
    process.exit(1);
  }
} else {
