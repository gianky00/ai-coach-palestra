...
Tip: **ops+full GREEN** on Pixel_9a at `4401a5f`. Privacy HTML draft GREEN `b072657`/`e6f1546`. Maestro e2e `17a978e`. Store smoke-clear harden `4c9ba47` (tip sync `a42bab3`). **Store shots BLOCKED — need-login:** Pixel_9a cleared (no SMOKE); `store:screenshots` → auth screen (`fail-need-login-*`). No real demo creds in env / `.env` / `.env.example` (VERIFY/Maestro only show placeholders `test@example.com` — do not invent). Next: set real `MAESTRO_TEST_EMAIL`/`MAESTRO_TEST_PASSWORD` (or manual demo login) → `npm run store:screenshots`; host privacy URL; e2e once Maestro on PATH. Do **not** delete syncFeedback.
