...
Tip: **CLAIMED — Maestro flow YAML harden + Gate F lint** (no adb). Store shots still **BLOCKED need-login** on Pixel (`a42bab3`/`4c9ba47` clear OK; auth `fail-need-login-*` — no invent demo creds). Prior: ops+full GREEN `4401a5f`; privacy HTML `b072657`/`e6f1546`; Maestro wrapper `17a978e`. Next after this ship: manual/demo login → `store:screenshots`; host privacy URL; e2e once Maestro on PATH. Prefer **not** Notifee `alarmManager`. Do **not** delete syncFeedback.
...
2. ~~**Trim unused Android permissions**~~ — done (`ac21d44`): main keeps `INTERNET` / `POST_NOTIFICATIONS` / `VIBRATE`; strips unused app perms + `tools:node=remove` for RNFS storage + Keychain biometric merges; debug keeps `SYSTEM_ALERT_WINDOW`; STORE_SUBMISSION Data safety table updated.
   ...
   2c. ~~**Store screenshot npm script**~~ — done (`a819a74`): `npm run store:screenshots` → `capture_store_screenshots.ps1` taps real tabs (no `kinefit://smoke/*`), aborts on SMOKE banner, writes `scripts/android/.store-shots/store-NN-*.png`. CI `31741590016` success.
   2c2. ~~**Store screenshots smoke-clear harden**~~ — done (`4c9ba47`): one-shot `kinefit://smoke/clear` → `am force-stop` → MainActivity (no `pm clear`); still aborts if SMOKE remains.
   2c3. **Store screenshots capture** — **BLOCKED need-login** on Pixel_9a/`emulator-5554`: smoke cleared (no banner); re-run FAIL auth (`fail-need-login-20260813-230156.*`). No `MAESTRO_TEST_*` / demo secrets available (docs placeholders only). Manual/demo login required before PASS assets.
   ...
   2g. **Maestro flow YAML harden + static lint** — WIP (this agent): smoke_all seed-less (week selector + empty/heatmap `when:`); smoke_ops settings sections + `timer=90` rest presets; navigation screen ids; `check-maestro-flows.ps1` + Gate F Python lint; `npm run maestro:flows-check`. No emulator / no Notifee alarmManager.
...
### 2026-08-13 — store:screenshots BLOCKED need-login (smoke cleared)

- Files touched: `docs/AGENT_SYNC.md` only (script harden already `4c9ba47`)
- Bugs fixed: leftover SMOKE on Pixel_9a cleared via `kinefit://smoke/clear` → force-stop → MainActivity (no `pm clear`)
- Notes: Re-ran `npm run store:screenshots -- -NoBoot` → **FAIL need-login** (0 store PNGs). Evidence: `scripts/android/.store-shots/fail-need-login-20260813-230156.{png,xml,log}`. No real demo/staging credentials in process env, `mobile/.env`, or `.env.example` — VERIFY/Maestro docs only show placeholders (`test@example.com` / `secret`); refused to invent. **Blocker:** operator must log in demo account (or export real `MAESTRO_TEST_EMAIL`/`MAESTRO_TEST_PASSWORD`) then re-run store shots. Do **not** commit `.store-shots` / delete syncFeedback. Env `KINEFIT_*` only. No Expo.
