...
Tip: Play store screenshots / privacy policy URL (`5d647d4`). Next: **enable Notifee `alarmManager` (+ `SCHEDULE_EXACT_ALARM`) if exact rest-timer needed** — or **P0 verify:ui:ops** (sibling owns Pixel) — or host real privacy HTML + set `KINEFIT_PRIVACY_POLICY_URL` (out-of-repo). Do **not** delete syncFeedback.
